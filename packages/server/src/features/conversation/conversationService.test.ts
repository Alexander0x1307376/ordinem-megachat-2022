...
describe('беседы на нескольких пользователей', () => {
  ...
  beforeAll(async () => {
    ...
    conversationService = container.get<IConversationService>(TYPES.ConversationService);
  ...
  test('беседа успешно создаётся', async () => {
    ...
    const conversationRow = await dataSource.createQueryBuilder(Conversation, 'c')
      .select('c.id AS id, c.uuid AS uuid')
      .where('c.uuid = :uuid', {uuid: conversation.uuid})
      .getRawOne();

    const chatRoomRow = await dataSource.createQueryBuilder(ChatRoom, 'c')
      .select('c.id AS id, c.uuid AS uuid')
      .where('c.uuid = :uuid', {uuid: conversation.chatRoomUuid})
      .getRawOne();

    const membersRows = await dataSource.createQueryBuilder()
  ...
  test('снос беседы, успех', async () => {
    ...
    const chatRoomRow = await dataSource.createQueryBuilder(ChatRoom, 'r')
      .select('r.id')
      .where((qb) => {
        const subQuery = qb.subQuery()
          .from(Conversation, 'c')
